...
namespace CASCLib
{
    ...
    public class WowRootHandler : RootHandlerBase
    {
        ...
        private ConcurrentDictionary<int, ulong> FileDataStore = new ConcurrentDictionary<int, ulong>();
        private ConcurrentDictionary<ulong, int> FileDataStoreReverse = new ConcurrentDictionary<ulong, int>();
        ...
        public WowRootHandler(BinaryReader stream, BackgroundWorker worker)
        {
            ...
            while (stream.BaseStream.Position < stream.BaseStream.Length)
            {
                ...
                Parallel.For(0, count, i =>
                {
                    ...
                    FileDataStore.TryAdd(fileDataId, hash);
                    FileDataStoreReverse.TryAdd(hash, fileDataId);
                    ...
                    //     FileDataStoreReverse.TryAdd(fileDataHash, fileDataId);
                    // }
                });

                worker?.ReportProgress((int)(stream.BaseStream.Position / (float)stream.BaseStream.Length * 100), "Loading root...");
            }
        }
        ...
        public override void LoadListFile(string path, BackgroundWorker worker)
        {
            ...
            using (var sr = new StreamReader(fs2))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    // worker?.ReportProgress((int)(sr.BaseStream.Position / (float)sr.BaseStream.Length * 100), "Loading listfile...");
                }
            }
        }
        ...
        protected override CASCFolder CreateStorageTree()
        {
            ...
            foreach (var rootEntry in RootData)
            {
                ...
                var hash = FileDataStore[rootEntry.Key];
